...
const HARD_MAX_REMINDERS = 1;
const REMINDER_ALLOWED_START_HOUR = Number.isFinite(env.reminderAllowedStartHour)
  ? env.reminderAllowedStartHour
  : 9;
const REMINDER_ALLOWED_END_HOUR = Number.isFinite(env.reminderAllowedEndHour)
  ? env.reminderAllowedEndHour
  : 19;
const REMINDER_TIMEZONE = env.reminderTimezone || 'America/Santiago';
...
function isInAllowedWindow(hour: number, startHour: number, endHour: number): boolean {
  if (startHour === endHour) return true;
  if (startHour < endHour) return hour >= startHour && hour < endHour;
  return hour >= startHour || hour < endHour;
}

function nextAllowedReminderTime(
  now: Date,
  timeZone: string,
  startHour: number,
  endHour: number
): Date {
  if (isInAllowedWindow(getHourInTimeZone(now, timeZone), startHour, endHour)) {
    return now;
  }
  ...
  for (let i = 0; i < 60 * 48; i += 1) {
    ...
    if (isInAllowedWindow(hour, startHour, endHour)) {
      return candidate;
    }
  }
...
async function processDueReminders() {
  ...
  for (const job of (jobs ?? []) as ReminderJob[]) {
    ...
    try {
      ...
      const effectiveMaxReminders = HARD_MAX_REMINDERS;

      if (lead.reminders_sent >= effectiveMaxReminders) {
        await Promise.all([
          service
            .from('reminder_jobs')
            .update({
          ...
          service
            ...
            .update({
              ...
              human_required_reason: 'NO_RESPONSE_TIMEOUT',
      ...
      const now = new Date();
      const localHour = getHourInTimeZone(now, REMINDER_TIMEZONE);
      if (
        !isInAllowedWindow(
          localHour,
          REMINDER_ALLOWED_START_HOUR,
          REMINDER_ALLOWED_END_HOUR
        )
      ) {
        const rescheduledFor = nextAllowedReminderTime(
          now,
          REMINDER_TIMEZONE,
          REMINDER_ALLOWED_START_HOUR,
          REMINDER_ALLOWED_END_HOUR
        ...
        await service
          ...
          .update({
            ...
            error_text: `Deferred by quiet hours (${REMINDER_ALLOWED_START_HOUR}:00-${REMINDER_ALLOWED_END_HOUR}:00 ${REMINDER_TIMEZONE})`
          })
          .eq('id', job.id)
          .eq('status', 'PENDING');
        summary.skipped += 1;
      ...
      const shouldCloseByNoResponse = nextRemindersSent >= effectiveMaxReminders;
      const nextReminderAt =
        shouldCloseByNoResponse
          ? null
      ...
      await Promise.all([
        ...
        service
          ...
          .update({
            ...
            ...(shouldCloseByNoResponse
              ? {
                  conversation_status: 'CLOSED' as const,
                  closed_at: new Date().toISOString(),
                  human_required_reason: 'NO_RESPONSE_TIMEOUT'
                }
              : {})
